...
pub trait PersistentStore {
    type ResourceUnit: Serialize + DeserializeOwned, /// type for a single serialize+store operation

    /// location for last confirmed atomic update; set when AtomicStore initiates load
    pub fn persisted_location(&self) -> Option<StorageLocation>;
    /// moving position as local store operations complete
    pub fn active_location(&self) -> Option<StorageLocation>;
    ...
    pub fn store_resource(&mut self, &str key, resource: &ResourceUnit) -> Result<StorageLocation, PersistenceError>;

    /// load the latest persisted unit of resource
    pub fn load_latest(&self) -> Result<ResourceUnit, PersistenceError>;
    /// load a specified unit of resource, if available
    pub fn load_specified(&self, location: &StorageLocation) -> Result<ResourceUnit, PersistenceError>;
}
...
impl AtomicStoreLoader {
    pub fn new(file_pattern: &str) -> Result<AtomicStoreLoader>;
}
